...
int         find_next_given_quote_index(char const *s, int i, char quote)
{
    i++;
    while(s[i] && s[i] != quote)
        i++;
...
int         word_counter(char *s)
{
    ...
    while (s[i])
    {
        if (is_quote(s[i]))
        {
            ret ++;
            mem = 0;
            i = find_next_given_quote_index(s, i, s[i]);
            if (i == -1)
                return (-1);
        }
        else if (mem == 0)
...
char        **fill_tab(char **tab, char const *s, int tab_size)
{
	...
	while (i < tab_size)
	{
        ...
        if (is_quote(s[j]))
            next_quote_index = find_next_given_quote_index(s, j, s[j]);
...
int main(int ac, char **av)
{
    ...
    ss = quote_spliter("\"t'e'g''\"i''werwer");
